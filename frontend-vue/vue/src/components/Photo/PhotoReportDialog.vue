  ...
  <v-row justify="center">
    <v-dialog v-model="dialog" persistent max-width="600px">
      <template v-slot:activator="{ on }">
        <v-btn class="ma-2" outlined color="yellow darken-1" v-on="on" @click="checkUser">
          <v-icon>mdi-alert</v-icon>
        </v-btn>
      </template>

      <v-card>
        <v-card-title class="headline font-weight-bold" primary-title>사진의 잘못된 부분을 알려주세요</v-card-title>
        <v-divider></v-divider>
        ...
        <v-card-actions>
          ...
          <v-btn
            ...
            @click="reportPhoto"
          >보내기</v-btn>
          <v-btn class="font-weight-black" color="primary" text @click="dialog = false">취소하기</v-btn>
        </v-card-actions>
      </v-card>
    </v-dialog>
  </v-row>
